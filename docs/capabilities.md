...
| integrate JUnit, TestNG, Page Objects, Allure, or parallel tests | [Framework integration](framework-integration.md) |
...
Lens is runner-agnostic. JUnit and TestNG retain test lifecycle and assertion ownership; setup code attaches Lens after driver creation and finalizes it in teardown. Existing Page Objects may accept/use `TestLens`, return `UiLocator`, or continue using raw Selenium beside Lens. Reports are independent artifacts and can be attached to Allure or another reporter. Create one Lens/session per test/driver for parallel execution; do not share mutable drivers or sessions across threads.

## What Test Lens does NOT do

- It does not create, configure, pool, or close `WebDriver` instances.
