...
Network assertions have an additional evidence precondition. `network.assertNoFailedRequests()` succeeds only for a capture generation that actually became active, including a valid generation stopped normally afterward. Never-started, `OFF`, unsupported, failed, and still-initializing capture cannot establish an absence of failures and throws `NetworkAssertionError`. Network waits retain their distinct immediate `SKIPPED`/`FAILED` results with zero attempts.
