...
Passive network capture is contained in the main Selenium module behind a package-private adapter. It creates one official Selenium 4.39 `Network` module per capture generation and subscribes to before-request, response-completed, and fetch-error events. Public Test Lens signatures do not expose beta BiDi types. Local `HasBiDi` connections are initialized through Selenium's lazy `getBiDi()` contract. A raw capability-backed `RemoteWebDriver` receives one private augmented view for the capture generation; the consumer's driver/decorator is not replaced, and a connection owned by that private view is closed with the source. Generation reservation under the lifecycle lock is the publication boundary: `stop()` monotonically invalidates an active or initializing generation, while completion publishes an adapter only if it still owns that generation. A stale adapter is closed exactly once outside the lock, and its callbacks or late initialization failure cannot overwrite a newer or stopped state. The adapter uses neither CDP nor performance logs and never closes the WebDriver session.
...
In the `0.3.0` line, immutable `HudOptions` is the product-level HUD contract. Runtime sinks apply the configured event-category visibility before `HudPanel` serializes visible rows into the canonical `hud-panel.js` runtime resource. The runtime, homepage demonstration, and HUD Studio all execute that same resource; documentation supplies synthetic events but does not maintain a second HUD implementation. Content visibility is a presentation boundary and does not remove events from trace or other sinks.
