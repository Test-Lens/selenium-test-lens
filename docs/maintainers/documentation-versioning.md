...
The operation checks that production Java sources and the public API manifest still match `v0.2.0`, creates a detached worktree from that tag, overlays only the homepage and demo files, and runs a strict stable build. The development homepage keeps its edit link to `main`; only the overlaid stable copy hides that link because `v0.2.0` contains the earlier homepage source. The iframe document is an isolated static asset rather than a MkDocs content page, so it intentionally has neither an edit link nor a second version switcher. The disposable worktree is force-removed in `finally` from its generated system-temporary path, so the intentional dirty overlay is also cleaned after build or publication failure.

Before any push, the publication script verifies that the Git object IDs for `/dev/`, `/0.1.0/`, and the root redirect have not changed, that `latest` is an exact copy of the repaired `0.2.0`, and that `mike` metadata still assigns the alias to `0.2.0`. A full MkDocs rebuild may change only `index.html`; the three demo files `demo/hud/{index.html,demo.css,demo.js}`; the three copied renderer files below `demo/hud/runtime/`; and homepage-derived global output `search/search_index.json`, `sitemap.xml`, and `sitemap.xml.gz`, under both `/0.2.0/` and its copied `/latest/` alias. The search index changes because it contains homepage text; sitemap output may change when MkDocs regenerates page metadata. Any change to another stable HTML page, unrelated theme asset or download, version metadata, or other generated file stops the operation before push. It never enables general stable-version replacement and cannot target another version.
