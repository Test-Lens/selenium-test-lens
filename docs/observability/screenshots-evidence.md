...
Full-page capture treats viewport width, viewport height, document width, device-pixel ratio, and top-level browsing-context identity as hard stitching invariants. A hard-invariant change retries the entire guarded capture once; the first partial image is discarded. Document height is instead a dynamic capture extent: growth appends newly required bottom positions, shrinkage clamps the remaining extent, actual browser scroll positions prevent duplicate bottom tiles, and two consecutive post-scroll geometry samples must confirm that the captured bottom is stable before final allocation and cropping.

Dynamic capture remains bounded by `maxPixelCount` (40 million by default), `maxTileCount` (200 by default), at most eight increases beyond the previous maximum document height, and a geometry-observation budget derived from the configured tile limit. Continued growth reports `DOCUMENT_HEIGHT_DID_NOT_STABILIZE`; pixel/tile extent limits report `CAPTURE_HEIGHT_LIMIT_EXCEEDED`; hard changes report `VIEWPORT_GEOMETRY_CHANGED`; and a PNG/tile failure reports `TILE_CAPTURE_FAILED`. Limits, non-stabilizing height, invalid PNG, or restoration failure are not retried and produce an explicit non-captured result with no partial final file.
