...
The standalone HTML has no external runtime dependency and works from `file://` as well as an HTTP server. Its main sections use the available viewport; long values stay inside their cards, code blocks, or table regions instead of widening the document. Event attributes are controlled from the event row and rendered in a full-width row below it. The control exposes `aria-expanded`/`aria-controls`, works from the keyboard, and uses report-scoped identifiers in suites. With JavaScript blocked, the native attribute `<details>` content remains present and operable.

An overflowing timeline has its own bounded vertical region with sticky column headings and native scrolling. A progressively enhanced horizontal scrollbar follows the overflowing timeline currently visible in the viewport and synchronizes `scrollLeft` in both directions. It hides when no visible timeline overflows, responds to resize/zoom and opened details, and leaves the native scrollbar as a fallback. LIGHT, DARK, and AUTO use the same semantic surfaces; AUTO follows `prefers-color-scheme` changes without regenerating or reloading the report.

HTML generated by an older Test Lens version contains that version's embedded markup and CSS. Upgrade and regenerate the report to receive renderer fixes; updating the JAR does not mutate archived HTML files.
