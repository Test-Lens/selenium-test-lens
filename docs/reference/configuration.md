...
`HighlightOptions` defaults are: enabled and automatic feedback on; ACTION `#ffeb3b`, WAITING `#38bdf8`, RETRY `#f59e0b`, SUCCESS `#22c55e`, FAILURE `#ef4444`; 1500 ms; 2 px border; labels on. `TestLensOptions.Builder.highlight(...)` installs the typed options. `OverlayConfig.highlightColor(...)` and `decorationDurationMs(...)` remain compatibility setters for action color and duration; explicit typed options are authoritative in either call order.
