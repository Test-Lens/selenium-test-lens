...
- a central-redaction canary crosses a BiDi query/header, API-overlay preview, step metadata, failure throwable, page source, HUD, trace, reports, and unpacked bundle; Chrome and Firefox must retain `[REDACTED]` while no text artifact contains the canary. The canary is never rendered as visible page content, because screenshots are intentionally outside text redaction.
