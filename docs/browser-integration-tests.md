...
- apostrophe-bearing and escaped-quote structured-JSON canaries cross a BiDi query/header, API-overlay request/response preview, step/trace metadata, failure throwable, page source, HUD, trace, reports, and unpacked bundle; Chrome and Firefox must retain `[REDACTED]` while no text artifact contains either full canary or its identifying fragment. The canaries are never rendered as visible page content, because screenshots are intentionally outside text redaction.
